...
static void SetPwm2(int EscSpeedCommand) {
    uint16 EscDutyCycle;
    if(EscSpeedCommand >= 0) {
        EscDutyCycle = (uint16)(EscInstance.MedDutyCycle + EscSpeedCommand*(int)(EscInstance.MaxDutyCycle-EscInstance.MedDutyCycle)/100);
        Pwm_SetDutyCycle(EscInstance.Channel2, EscDutyCycle);
    }
    else{
        EscDutyCycle = (uint16)(EscInstance.MedDutyCycle + EscSpeedCommand*(int)(EscInstance.MedDutyCycle-EscInstance.MinDutyCycle)/100);
        Pwm_SetDutyCycle(EscInstance.Channel2, EscDutyCycle);
...
void Esc_Period_Finished(void){
   ...
   void Esc_Period_Finished2(void){
        int EscSpeedCommand;
        /*if your chosen ESC has braking capabilities, set this on STD_ON. If not, set it on STD_OFF.
         * Using the wrong configuration can make the car go full speed backward on braking!*/
    #if (ESC_HAS_BRAKE == STD_ON)
        /* state machine updates here at every PWM signal edge*/
        switch(EscInstance.State2){
        case Forward:
            if(EscInstance.Brake != 0U || EscInstance.Speed2 < 0){
                EscInstance.State2 = Braking;
            }
            break;
        case Braking:
            if(EscInstance.Brake == 0U && EscInstance.Speed2 >= 0){
                EscInstance.State2 = Forward;
            }
            else if(EscInstance.Brake == 0U && EscInstance.Speed2 < 0){
                EscInstance.State2 = Neutral;
            }
            break;
    #else
        switch(EscInstance.State2){
            case Forward:
                ...
                else if(EscInstance.Speed2 < 0){
                    EscInstance.State2 = Reverse;
                }
                break;
            case Braking:
                if(EscInstance.Brake != 0U){
        ...
        switch(EscInstance.State2){
            case Forward:
            case Reverse:
                EscSpeedCommand = EscInstance.Speed2;
                break;
            case Braking:
    #if (ESC_HAS_BRAKE == STD_ON)
                EscSpeedCommand = -100;
        ...
        SetPwm2(EscSpeedCommand);
    }
/*==================================================================================================
*                                       GLOBAL FUNCTIONS
==================================================================================================*/

void EscInit(Pwm_ChannelType EscPwmChannel,Pwm_ChannelType EscPwmChannel2, uint16 MinDutyCycle, uint16 MedDutyCycle, uint16 MaxDutyCycle){
    EscInstance.Channel = EscPwmChannel;
    EscInstance.Channel2 = EscPwmChannel2;
    ...
    Pwm_SetDutyCycle(EscInstance.Channel2, EscInstance.MedDutyCycle);
    ...
    Pwm_EnableNotification(EscInstance.Channel2, PWM_FALLING_EDGE);
}
...
void EscSetSpeed2(int Speed){
    if(Speed > 100){
        EscInstance.Speed2 = 100;
    }
    else if(Speed < -100){
        EscInstance.Speed2 = -100;
    }
    else{
        EscInstance.Speed2 = Speed;
    }
    EscInstance.Speed2=-EscInstance.Speed2;
}
